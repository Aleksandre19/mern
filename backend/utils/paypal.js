...
// Checks if a PayPal transaction is new by comparing the
// transaction ID with existing orders in the database.
export async function checkIfNewTransaction(orderModel, paypalTransactionId) {
  try {
    // Find all documents where Order.paymentResult.id is the
    // same as the id passed paypalTransactionId
